...
export async function getAdminProducts(params?: Record<string, any>) {
  const cacheKey = `admin:products:${JSON.stringify(params || {})}` || "admin:products:default";
  const cached = getCached(cacheKey);
  if (cached) return cached;
  try {
    const candidates = [
    ...
    const data = await tryCandidates(candidates, { method: "get", params });
    setCached(cacheKey || "admin:products:default", data);
    return data;
  } catch (e) {
    return handleAxiosError(e);
  }
...
export async function getSupplierImportTaskStatus(taskId: string) {
  try {
    const res = await axiosInstance.get(`/api/admin/supplier-intelligence/tasks/${encodeURIComponent(taskId)}`);
    return res.data;
  } catch (e) {
    return handleAxiosError(e);
  }
...
api.getSupplierImportTaskStatus = getSupplierImportTaskStatus;
