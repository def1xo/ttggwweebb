...
def test_rerank_gallery_images_shop_vkus_drops_first_two_when_total_seven():
...
def test_import_products_shop_vkus_stock_range_does_not_auto_mark_all_sizes_in_stock(monkeypatch):
    engine = create_engine("sqlite:///:memory:", future=True)
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    db = Session()
...
def test_import_products_shop_vkus_stock_word_in_stock_does_not_mark_all_sizes(monkeypatch):
    engine = create_engine("sqlite:///:memory:", future=True)
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine, autoflush=False, autocommit=False)
    db = Session()
